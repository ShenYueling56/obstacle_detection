...
//    cout<< R << endl<< T << endl << cameraMatrix_L << endl << distCoeffs_L << endl << cameraMatrix_R << endl<< distCoeffs_R<<endl;
    ...
    while (getline(imageStore_L, imageName_L))
    {
        ...
        Mat gd_sparity(640-245, 480, CV_32FC1, Scalar::all(0));;
...
//        }
        ...
        show_bbox_on_sparity(root_path, bbox_list_after_screen_l, disparity_l, count);
        cout<<"finish show bbox on sparity"<<endl;
        //Step8 利用视差图和投影参数Q得到三维图像
        cout<<"Q"<<endl<<Q<<endl;
        reprojectImageTo3D(disparity_l, result3DImage, Q);
        imshow("disparity", disparity_l);
        setMouseCallback("disparity", onMouse);
        if (waitKey(0) == 27) {
            destroyAllWindows();
        }
//        Mat filtered_disparity;
//        filterDisparityImage(root_path, rectified_l, disparity_l, disparity_r, filtered_disparity, count);
//        reprojectImageTo3D(filtered_disparity, result3DImage, Q);
//        imshow("filtered_disparity", filtered_disparity);
//        setMouseCallback("filtered_disparity", onMouse);
//        if (waitKey(0) == 27) {
//            destroyAllWindows();
//        }
        ...
        if(obj_det_3d(root_path, bbox_list_after_screen_l, rectified_l, result3DImage, bbox_3d_list, count, imageSize, P1))
        {
        ...
        {
            cout<<"finish 3d object detection"<<endl;
            cout<<"find no object cluster"<<endl;
            fResult<<count<<" -1 0 0 0 0"<<endl;
        }
        count = count + 1;
    }
