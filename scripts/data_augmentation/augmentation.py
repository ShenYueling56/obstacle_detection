...
# todo:  2.旋转、翻转、缩放 3.挑选视角更好的背景图片  4.应对拥挤，遮挡：如果两个框的iou大于阈值，就随机删除一个框
import os
import random
import cv2 as cv
import numpy as np
...
if __name__ == '__main__':
    ...
    classes = ["badminton/", "wire/", "mahjong/", "dogshit/"]
    ...
    # wire_path = obj_path + "wire/"
    # badminton_path = obj_path + "badminton/"
    # mahjong_path = obj_path + "mahjong/"
    # dogshit_path = obj_path + "dogshit/"
            ...
            for i in range(4): # for each class
                ...
                for j in idx_list:
                    ...
                    if area < 400:
                        ...
                        continue
                    if area>1500:
                        idy = int_random(3*bk_h/4-h, bk_h - h, 1)[0]
                    else:
                    ...
                    if boxes.shape[0] == 0:
                        ...
                        boxes_after, cs, last_box_del, obj_list= my_delete_box(boxes, boxes, cs, iou, obj_list)
                        obj_index = obj_index - (boxes.shape[0]-boxes_after.shape[0])
                        boxes = boxes_after
                        # print(boxes)
                        # print(cs)
                        # print(last_box_del)
                    ...
                    # if not last_box_del:
                    #     result = bk
                    #     obj_grey = cv.cvtColor(obj, cv.COLOR_BGR2GRAY)#
                    ...
                    #     result[idy:idy+h, idx:idx+w, :] = img_bg + obj
